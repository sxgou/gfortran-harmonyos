...
### 9.10 git push 需要 API 脚本

HarmonyOS 自带的 git 缺少 `remote-https` 插件，且 `ssh` 被 hmmac 拦截。无法使用标准的 `git push` 命令。替代方案是使用 curl + GitHub API：

```bash
python3 /data/storage/el2/base/git-update2.py
```

该脚本会读取本地 git 仓库的最新提交，通过 GitHub Data API 创建 blob → tree → commit → 更新 ref。
